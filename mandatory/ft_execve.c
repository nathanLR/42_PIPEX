...
/*   Updated: 2024/01/17 23:11:21 by nle-roux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execve(char **cmd, t_data *data, char **env)
{
	...
	if (ft_strchr(cmd[0], '/') != NULL)
	{
		if (execve(cmd[0], cmd, env) == -1)
			ft_manage_error(ft_strjoin(E_NOT_EXISTS, cmd[0]), U_ERROR, data, C_CMD_NOT_FOUND);
	}
	else
	{
		path = ft_get_cmd_path(cmd[0], env);
		if (path == NULL)
			ft_manage_error(ft_strjoin(E_CMD_NOT_FOUND, cmd[0]), U_ERROR, data, C_CMD_NOT_FOUND);
